...
- 燃油&纯电和续航信息
- 支持动力电池电量、纯电续航、充电状态和 TM/TA 行程信息
- 车辆位置跟踪
- 车辆警告信息（保养、液位、胎压）
- 支持充电桩控制
- T8 满电续航长期统计
...
- 登录会话跨重启持久化，重启优先用 refresh token 续期，减少密码登录触发风控的概率；密码登录本地限流（120 秒冷却）。
- 服务器提前作废会话（401/403）时同一轮更新内自动重新认证；确定性 4xx 错误不重试。
- 网络故障时最多 5 轮内继续提供缓存数据，之后实体变为不可用，恢复后自动回归；不再出现单个数据源故障导致锁/里程/位置显示默认值的问题。
- 密码失效时 Home Assistant 会弹出"需要重新认证"通知，在集成页面重新输入密码即可，无需删除重装。
- 日志已全面脱敏（token 不再出现在异常堆栈中）。若你曾对外分享过旧版本日志，建议在沃尔沃 App 修改一次密码。
- 升级注意事项（含 entity_id 小写化说明）见 [docs/releases/v3.2.0.md](docs/releases/v3.2.0.md)。
...
`model` 可选 `s90_t8`、`s90`、`xc60_t8`、`xc60`、`xc90_t8`、`xc90` 或 `generic`。如需使用自己的俯视图，可通过卡片编辑器填写 `/local/...` 或 HTTPS 图片地址；自定义图片应保持车头朝上、完整车辆居中的纵向俯视构图。

卡片展示车辆电量、燃油量、双能源续航、最近满电续航、TM/TA 行程和常用远程控制。远程指令发送期间会锁定对应按钮以避免重复操作，并显示成功或失败反馈；Home Assistant 中状态为 `unknown` 的按钮实体仍可正常执行。充电功率、预计充满时间及充电桩名称/地址暂不放入卡片，相关数据仍可通过独立实体使用。
